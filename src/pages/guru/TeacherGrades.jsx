...
const TeacherGrades = () => {
  ...
  const handleGradeChange = async (studentId, type, value) => {
    ...
    const updatedGrade = {
      ...
      total: calculateTotalGrade(
        type === 'assignment' ? value : (currentGrade.assignment || ''),
        type === 'midterm' ? value : (currentGrade.midterm || ''),
        type === 'final' ? value : (currentGrade.final || '')
      )
    };

    try {
      await firestoreService.updateDocument('grades', gradeId, updatedGrade);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Input Nilai Siswa</h2>
        <p className="text-gray-600 mt-1">Input dan perbarui nilai untuk mata pelajaran yang Anda ajar</p>
      </div>
